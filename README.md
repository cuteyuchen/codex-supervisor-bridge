...
> **Experimental runtime safety status (2026-08-31):** Remote Profile B is not
...
The current P6.6 blocker is confirmed Windows AppData execution-context
redirection: the same requested AppData path can resolve to different physical
files when launched from different Windows contexts, including a context that
reports `NO_PACKAGE_IDENTITY`. The branch is adding a standalone Supervisor
Host and physical-path guard. This is experimental code only; it does not yet
claim Desktop runtime isolation or production-safe Remote Profile B.
